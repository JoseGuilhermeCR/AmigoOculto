...
/*********
 * ARVORE B+ SI
 * String chave1, int chave2
 * 
 * Os nomes dos métodos foram mantidos em inglês
 * apenas para manter a coerência com o resto da
 * disciplina:
 * - boolean create(String chave1, int chave2)
 * - int[] read(String chave1)
 * - boolean delete(String chave1, int chave2)
 * 
...
// Árvore B+ para ser usada como índice invertido com repetições
// CHAVE1: String - usado para algum atributo textual da entidade (Nome, Email, ...) que pode ser repetido
// CHAVE2: Int    - usado para o ID da própria entidade (ou de outra entidade qualquer)

public class ArvoreBMais_ChaveComposta_String_Int {
    ...
    private class Pagina {
        ...
        protected String[] chaves1;               // Chave principal
        protected int[]    chaves2;               // Chave subordinada
        ...
        protected int      TAMANHO_CHAVE1;        // Tamanho da string máxima usada como chave
        ...
        public Pagina(int o) {
            ...
            maxFilhos = o;
            maxElementos = o-1;
            chaves1 = new String[maxElementos];
        ...
        private byte[] completaBrancos(String str) {
            byte[] aux;
            byte[] buffer = new byte[TAMANHO_CHAVE1];
            aux = str.getBytes();
            int i=0; while(i<aux.length) { buffer[i] = aux[i]; i++; }
            while(i<TAMANHO_CHAVE1) { buffer[i] = 0x20; i++; }
            return buffer;
        }
        ...
        protected byte[] getBytes() throws IOException {
            ...
            while(i<n) {
                ...
                out.write(completaBrancos(chaves1[i]));
        ...
        public void setBytes(byte[] buffer) throws IOException {
            ...
            byte[] bs = new byte[TAMANHO_CHAVE1];
    ...
    public ArvoreBMais_ChaveComposta_String_Int(int o, String na) throws IOException {
    ...
    public int[] read(String c1) throws IOException {
    ...
    private int[] read1(String chave1, long pagina) throws IOException {
        ...
        // Encontra o ponto em que a chave1 deve estar na página
        // Primeiro passo - todas as chaves1 menores que a chave1 buscada são ignoradas
        int i=0;
        while(i<pa.n && chave1.compareTo(pa.chaves1[i])>0) {
            i++;
        }
        
        // Chave encontrada (ou pelo menos o ponto onde ela deveria estar).
        // Segundo passo - testa se a chave1 é a chave1 buscada e se está em uma folha
        // Obs.: em uma árvore B+, todas as chaves1 válidas estão nas folhas
        // Obs.: a comparação exata só será possível se considerarmos a menor string
        //       entre a chave1 e a string na página
        if(i<pa.n && pa.filhos[0]==-1 
                  && chave1.compareTo(pa.chaves1[i])==0) {

            // Cria a lista de retorno e insere as chaves secundárias encontradas
            ArrayList lista = new ArrayList();
            while(chave1.compareTo(pa.chaves1[i])<=0) {
                
                if(chave1.compareTo(pa.chaves1[i])==0)
        ...
        // Terceiro passo - ainda não é uma folha, continua a busca recursiva pela árvore
            ...
            i=0;
            if(chave1.compareTo(pa.chaves1[0])<=0) {
        ...
        if(i==pa.n || chave1.compareTo(pa.chaves1[i])<0)
    ...
    public boolean create(String c1, int c2) throws IOException {

        // As chaves não pode ser vazias ou negativas
        if(c1.compareTo("")==0) {
            System.out.println( "A chave 1 não pode ser vazia" );
            return false;
        }
        if(c2<0) {
            System.out.println( "A chave 2 não podem ser negativas" );
            return false;
        }
        ...
        // e chave2Aux. Quando há uma divisão, a chave e o valor promovidos são armazenados
        // nessas variáveis.
        chave1Aux = c1;
        chave2Aux = c2;
    ...
    private boolean create1(long pagina) throws IOException {
        ...
        int i=0;
        while(i<pa.n && (chave1Aux.compareTo(pa.chaves1[i])>0 || (chave1Aux.compareTo(pa.chaves1[i])==0 && chave2Aux>pa.chaves2[i]))) {
            i++;
        }
        
        // Testa se a chave já existe em uma folha. Se isso acontecer, então 
        // a inclusão é cancelada.
        if(i<pa.n && pa.filhos[0]==-1 && chave1Aux.compareTo(pa.chaves1[i])==0 && chave2Aux==pa.chaves2[i]) {
            cresceu = false;
            return false;
        }
        ...
        boolean inserido;
        if(i==pa.n || chave1Aux.compareTo(pa.chaves1[i])<0 || (chave1Aux.compareTo(pa.chaves1[i])==0 && chave2Aux<pa.chaves2[i]))
        ...
        // Caso 2 - Novo registro deve ficar na página da direita
            ...
            int j;
            for(j=maxElementos-meio; j>0 && (chave1Aux.compareTo(np.chaves1[j-1])<0 || (chave1Aux.compareTo(np.chaves1[j-1])==0&&chave2Aux<np.chaves2[j-1]) ); j--) {
    ...
    public boolean delete(String chave1, int chave2) throws IOException {
    ...
    private boolean delete1(String chave1, int chave2, long pagina) throws IOException {
        ...
        // Encontra a página em que a chave1 está presente
        // Nesse primeiro passo, salta todas as chaves1 menores
        int i=0;
        while(i<pa.n && (chave1.compareTo(pa.chaves1[i])>0 || (chave1.compareTo(pa.chaves1[i])==0 && chave2>pa.chaves2[i]))) {
            i++;
        }

        // Chaves encontradas em uma folha
        if(i<pa.n && pa.filhos[0]==-1 && chave1.compareTo(pa.chaves1[i])==0 && chave2==pa.chaves2[i]) {
        ...
        // Se a chave1 não tiver sido encontrada (observar o return true logo acima),
        // continua a busca recursiva por uma nova página. A busca continuará até o
        // filho inexistente de uma página folha ser alcançado.
        ...
        if(i==pa.n || chave1.compareTo(pa.chaves1[i])<0 || (chave1.compareTo(pa.chaves1[i])==0 && chave2<pa.chaves2[i])) {
    ...
    private void print1(long pagina) throws IOException {
        ...
        for(i=0; i<maxElementos; i++) {
            System.out.print("("+String.format("%04d",pa.filhos[i])+") "+pa.chaves1[i]+","+String.format("%2d",pa.chaves2[i])+" ");
        }
