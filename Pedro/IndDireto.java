/*********
 * TABELA HASH EXTENSVEL
 * int chave, long dado
 * 
 * Os nomes dos mtodos foram mantidos em ingls
 * apenas para manter a coerncia com o resto da
 * disciplina:
...
class IndDireto {
    ...
    class Cesto {
        ...
        short quantidadeMaxima; // quantidade mxima de pares que o cesto pode conter
        int[] chaves; // sequncia de chaves armazenadas no cesto
        long[] dados; // sequncia de dados correspondentes s chaves
    ...
    class Diretorio {
        ...
        protected int hash2(int chave, int pl) { // clculo do hash para uma dada profundidade local
            return chave % (int) Math.pow(2, pl);
        }

    }

    public IndDireto(int n, String nd, String nc) throws Exception {
        ...
        // Se o diretrio ou os cestos estiverem vazios, cria um novo diretrio e lista
        // de cestos
        if (arqDiretorio.length() == 0 || arqCestos.length() == 0) {

            // Cria um novo diretrio, com profundidade de 0 bits (1 unico elemento)
            diretorio = new Diretorio();
            byte[] bd = diretorio.toByteArray();
            arqDiretorio.write(bd);

            // Cria um cesto vazio, j apontado pelo unico elemento do diretrio
    ...
    public boolean create(int chave, long dado) throws Exception {
        ...
        // Testa se a chave j nao existe no cesto
        if (c.read(chave) != -1)
            throw new Exception("Chave ja existe");

        // Testa se o cesto j nao est cheio
        ...
        // Atualiza o arquivo do diretrio
    ...
    // Mtodo principal apenas para testes
    public static void main(String[] args) {
        ...
        try {
            he = new IndDireto(4, "diretorio.hash.db", "cestos.hash.db");
